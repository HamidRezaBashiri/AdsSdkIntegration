...
package com.mobileaddemo.ads.prebid.MyPrebidWrapper;
 ...
 */
public class MyPublisherAdViewWrapper extends AdListener implements AppEventListener {

    private static final String TAG = MyPublisherAdViewWrapper.class.getSimpleName();

    private final AdManagerAdView adView;
    private final MyGamAdEventListener listener;

    private MyPublisherAdViewWrapper(
            Context context,
            String gamAdUnit,
            MyGamAdEventListener eventListener,
            AdSize... adSizes
    ) {
    ...
    @Nullable
    static MyPublisherAdViewWrapper newInstance(Context context, String gamAdUnitId,
                                                MyGamAdEventListener eventListener, AdSize... adSizes) {
        try {
            return new MyPublisherAdViewWrapper(context,
                    gamAdUnitId,
                    eventListener,
                    adSizes);
        }
    ...
    public void onAppEvent(
        ...
        if (Constants.APP_EVENT.equals(name)) {
            listener.onEvent(MyAdEvent.APP_EVENT_RECEIVED);
        }
    }
    //endregion ==================== GAM AppEventsListener Implementation

    //region ==================== GAM MyAdEventListener Implementation
    @Override
    public void onAdClosed() {
        listener.onEvent(MyAdEvent.CLOSED);
    }
    ...
    public void onAdFailedToLoad(
        ...
        final MyAdEvent MyAdEvent = com.mobileaddemo.ads.prebid.MyPrebidWrapper.MyAdEvent.FAILED;
        MyAdEvent.setErrorCode(loadAdError.getCode());

        listener.onEvent(MyAdEvent);
    }

    @Override
    public void onAdOpened() {
        listener.onEvent(MyAdEvent.CLICKED);
    }

    @Override
    public void onAdLoaded() {
        listener.onEvent(MyAdEvent.LOADED);
    }
    //endregion ==================== GAM MyAdEventListener Implementation

    public void loadAd(Bid bid) {
        try {
            AdManagerAdRequest adRequest = new AdManagerAdRequest.Builder()
                    .addCustomTargeting("hb_service_provider", "shinka_mobile")
                    .addCustomTargeting("hb_bidid", bid.getId())
                    .build();
